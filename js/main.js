function updategen(t) {
    ...
    function lpCreatePass(length, upper, lower, digits, special, mindigits, ambig, reqevery, pronounceable) {
      ...
      if (pronounceable) {
        if (upper)
          return GPW.pronounceablecaps(length);
        else
  ...
  function dogenerate() {
    ...
    if (!(/^\d*(?:\.\d{1,2})?$/).exec(length) || !(/^\d*(?:\.\d{1,2})?$/).exec(mindigits)) {
      error_msg += "Please only use digits in our Password Length and Minimum Numeric Characters fields.";
    } else {
      length = parseInt(length, 10);
      mindigits = parseInt(mindigits, 10);

      if (length < 1 || length > 256) {
        error_msg += "Please keep your password length to be between 1-256.<br/><br/>";
      }

      if (mindigits < 0) {
        error_msg += "Please keep your minimum numeric characters to be at least 0.<br/><br/>";
      }

      if (mindigits > length) {
        error_msg += "Please do not let your minimum numeric characters exceed your password length.<br/>";
      }
    }
    ...
    if (error_msg != "") {
      ...
      var newpw = lpCreatePass(length, upper, lower, digits, special, mindigits, ambig, reqevery, pronounceable);
      ...
      if (strength > (maxwidth * .60)) {

      } else if (strength >= (maxwidth * .30)) {
